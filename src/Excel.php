<?php
/**
 * excel文件导入&导出，需要安装PhpSpreadsheet包 -> composer require phpoffice/phpspreadsheet
 *
 * Created by PhpStorm.
 * User: xiajunwei
 * Date: 2019/5/5
...
class Excel
{
     ...
     *
     * @return array 返回excel数据
     * @throws Exception
     */
    public static function import($file = '', $sheet = 0, $columnCnt = 0, &$options = [])
    {
        try {
     ...
     */
    public static function export(array $datas, $fileName = '', array $options = [])
    {
        try {
            if (empty($datas)) {
